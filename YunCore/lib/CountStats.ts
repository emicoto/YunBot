...
export async function CountStats(ctx: Context, uid: string, mode?) {
	...
	let base = {
		...
		HP:(10 + level * 4.75) * f.LevelBuff(level),
		SP:(5 + level * 1.5) * f.LevelBuff(level),

		ATK:(5 + Math.floor(level/2)) * f.LevelBuff(level),
		DEF:(5 + Math.floor(level/2)) * f.LevelBuff(level),
		SPD:(5 + Math.floor(level/2)) * f.LevelBuff(level),
	...
	//最后做战力计算
	base.BP += base.HP/10 + base.SP/20 + base.ATK/2, base.DEF/5 + base.SPD/2 + add.BP
	...
	data.maxHP = Math.max(Math.floor(base.HP/5+0.5)*5,25)
	data.maxSP = Math.max(Math.floor(base.SP/5+0.5)*5,10)
