...
export const Res = [
  ...
  },
  {
    ...
    re: (str, uid, ctx) => {
      ...
      if (f.random(100) < 60) {
        return f.either([
          `……晚安，${uid == s.master ? "师父" : f.At(uid)}`,
          `……要睡了吗？那，${uid == s.master ? "师父" : f.At(uid)}晚安。`,
        ]);
      }
  ...
  },
  {
    ...
    re: (str, uid, ctx) => {
      ...
      if (
        ...
        (Date().includes("Feb 13") && new Date().getHours() > 9)
      )
        return txt;
  ...
  },
  {
    ...
    re: (str, uid, ctx) => {
      if (str.match(/睡|困|起来|走|累|死|来|去|回|老/)) return "next";
